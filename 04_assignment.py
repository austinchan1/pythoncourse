...
class PandasChain:
    ...
    def display_chain(self): 
        ...
        #loops through the chain and calls display_transactions method
        for block in self.__chain:
            
            block.display_transactions()
        
        #displays transaction for the current block
    ...
    def __commit_block(self,block): 
        ...
        #creates block hash using block metadata
        block_string = str(self.__prev_hash) + str(self.__seq_id) + str(block.get_simple_merkle_root())
        block_hash = hashlib.sha256(bytes(block_string,"utf-8")).hexdigest()# Create block hash
        
        #sets block hash and changes status to committed
        ...
        #updates previous hash, sequence id, and current block
    ...
    def display_block_headers(self): 
        ...
        #loops through blocks in chain and displays their headers
        for block in self.__chain:
            
            block.display_header()
            
        #displays header of current block
    ...
    def get_number_of_blocks(self): 
        ...
        #returns the number of committed blocks in chain and + 1 for current block
    ...
    def get_values(self):
        ...
        #list comprehension of transaction dataframes for each block
        value_dfs = [block.get_values() for block in self.__chain]
        
        #appends current block to transaction dataframe list
        value_dfs.append(self.__current_block.get_values())
        
        #concatenates all transaction dataframes into one big dataframe and returns it
        values = pd.concat(value_dfs,ignore_index = True)
...
class Block:
    ...
    def display_header(self): 
        ...
        #prints block metadata as strings separated by spaces
        print(str(self.__seq_id),
              str(self.__status),
              str(self.__block_hash),
              str(self.__prev_hash),
              str(self.__merkle_tx_hash),
              str(self.__transactions.shape[0]),
              sep = " ")
    
    # 10 pts - This is the interface for how transactions are added
    def add_transaction(self,s,r,v): 
        ts = str(dt.datetime.now()) # Get current timestamp 
        tx_hash = hashlib.sha256(bytes(ts + s + r + str(v),"utf-8")).hexdigest() # Hash of timestamp, sender, receiver, value
        ...
        new_transaction.loc[0] = [ts,s,r,v,tx_hash]
        # Append to the transactions data
        self.__transactions = self.__transactions.append(new_transaction).reset_index(drop = True)
    ...
    def get_size(self): 
        ...
        #returns number of rows in transaction dataframe using the shape method
    ...
    def get_simple_merkle_root(self): 
        ...
        #joins all transaction hashes into one long string and hashes the resulting string to create merkle root
        merkle_long_string = ''.join(str(item) for item in self.__transactions['TxHash'].tolist())
        merkle_root = hashlib.sha256(bytes(merkle_long_string,"utf-8")).hexdigest()
        
        
        #updates block's merkle root
        self.__merkle_tx_hash = merkle_root
        
        return(merkle_root)
    
    def get_values(self):
        
        #returns a dataframe of transaction timestamps and values
        return(self.__transactions[['Timestamp','Value']])
        

#Exercise 2

block = PandasChain("test")

block.add_transaction('a','b',1)
block.add_transaction('a','b',2)
block.add_transaction('a','b',3)
block.add_transaction('a','b',4)
block.add_transaction('a','b',5)
block.add_transaction('a','b',6)
block.add_transaction('a','b',7)
block.add_transaction('a','b',8)
block.add_transaction('a','b',9)
...
test_block = block.get_values()

plt.scatter(range(len(test_block['Value'])), test_block['Value'])
plt.show()


#Extra credit

plt.scatter(test_block['Timestamp'], test_block['Value'])
plt.show()
